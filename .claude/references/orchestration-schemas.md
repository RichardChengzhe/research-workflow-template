...
findings:
  - id: F1                       # stable within this run (lens-prefixed is fine: M1, P3)
    ...
    location: "Sec 4.2, Table 2 col 3"   # where in the artifact (page/section/line/cell)
    finding: "Standard errors are clustered by firm only, but treatment is assigned at the state-year level."
    evidence: "Table 2 note says 'SE clustered by firm'; the policy shock in Sec 3 varies at state×year."
    recommendation: "Cluster at the state level (or two-way state-and-year); report whether inference survives."
    change_my_mind: "A two-way-clustered column in Table 2 showing the t-stat holds."
...
For parity/gate skills (`/check`, `/disclosure-check`, `/audit-reproducibility`), the lens verdict is the hard-gate roll-up: `APPROVED` iff every hard gate passes (zero CRITICAL), else `BLOCKED`.
...
A synthesizer/editor/judge reduces lens findings — it must not **introduce** a blocking claim no lens raised. (An `editor` could otherwise desk-reject on a reason neither referee gave.) The gate:
...
This is cheap (it runs only on judge-introduced CRITICALs, usually 0–2) and it is the single most important guard for trusting an autonomous review near a credibility-sensitive artifact (a manuscript headed to a referee, a replication package headed to a data editor).

## 5. `RUN_CONFIG` — the pre-run input contract

A fan-out runtime collects every interactive choice **before** launch, so no subagent stalls waiting on the user mid-run (subagents cannot prompt). See [`.claude/rules/orchestrator-protocol.md`](../rules/orchestrator-protocol.md) → "RUN_CONFIG".

```yaml
run_config:
  artifact: manuscript/main.tex            # what is being reviewed
  ...
  cross_artifact: true                     # auto-invoke the paper↔code cross-check (see cross-artifact-review.md)
...
- [`agent-fleet.md`](agent-fleet.md) — which agent fills which lens, at which model tier.
