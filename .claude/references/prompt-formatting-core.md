...
Applied by the [`prompt-shaping`](../rules/prompt-shaping.md) rule as a standing habit. (There is no `/prompt` command — a loop-first workflow shapes every ambiguous request automatically rather than behind a slash command. This file is the shared skeleton that rule reaches for.) Defines the **structured-prompt skeleton** and the **depth-calibration heuristic**.
...
- Specific (good): "You are a senior finance referee for the *Journal of Financial Economics*, calibrated to its identification-credibility bar."
...
- The methodological tradition (reduced-form / structural / asset-pricing-test / theory+empirics / etc.).
...
- "MUST NOT recommend an instrumental-variables design — the user has already considered and ruled out IV."
...
The bookend catches the cases where the prompt was almost-but-not-quite specific enough. Without it, Claude guesses; with it, you get one focused question that improves the rest of the run.

---

## Depth calibration

The depth heuristic picks a default register for a shaped prompt; the user may override with `depth: light` / `standard` / `deep`.
...
- Use case: "Reword this paragraph for clarity." / "What's a good Stata command for X?"

### Standard

- Heuristic: input is 40–200 words OR contains 1+ domain-specific terms (econometrics / finance / accounting / methods jargon).
...
- Use case: "Help me think through the identification strategy for this event study."
...
If the user types `depth: light` / `standard` / `deep`, honor it. The heuristic is a sensible default, not a contract.
...
**Depth inference:** Standard (input is ~50 words, mentions a specific econometrics concept "classical measurement error", and asks for a response document, not a quick reword).
...
- A **`council` token** that dispatches the formatted prompt to Blattman's `/council` skill (his five-critic multi-model deliberation). This template doesn't ship `/council` — it has [`/seven-pass-review`](../skills/seven-pass-review/SKILL.md) and [`/devils-advocate`](../skills/devils-advocate/SKILL.md) instead, which serve adjacent purposes with different shapes. The closest council-style fit is the five-critic [`/promote-memory`](../skills/promote-memory/SKILL.md) council (the `promote-memory-council` agent).

If you fork this template and add your own multi-vendor or multi-model fan-out, extend this file with your routing rules — keep them in this reference so the `prompt-shaping` habit stays aligned.

---

## Boundary with `/interview-me`

Prompt-shaping is **single-shot input shaping** — one informal request becomes one formatted prompt, then its first execution. There is no multi-turn elaboration.

[`/interview-me`](../skills/interview-me/SKILL.md) is **multi-turn project specification** — a fuzzy research idea becomes a structured research spec (RQ, hypotheses, identification strategy, data needs, empirical strategy) via Socratic Q&A. The output is a spec file used by `/preregister`, `/research-ideation`, `/data-analysis`, etc.

They don't compete. Shape a prompt when you want a well-formed ask; use `/interview-me` when you want a well-formed research project. The former takes 30 seconds; the latter takes 10–20 minutes.
