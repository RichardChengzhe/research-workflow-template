...
Symmetric partner to the Pre-Flight input check ([`content-invariants.md`](content-invariants.md) + skill-level `## Phase 0`). Where Pre-Flight proves inputs were read *before* work, Post-Flight proves the output's factual claims hold *after* drafting -- before the skill returns to the user.
...
| `/review-paper` (`--peer` mode) | Novelty-probe claims from web search ("this paper's contribution is novel" / "similar to Jones 2020") |
| `/interview-me` | Papers referenced in the research spec (if any cited) |

Does **not** apply to mechanical skills (`/compile-latex`, `/run`, `/format-table`, `/commit`) -- they produce compiled or file output verified by an external process (the compiler, the script log), not factual text.
...
- **Existence claims:** "A dataset called X contains Y fields." / "WRDS exposes table Z."
- **Numerical facts:** "N = 10,000" / "coefficient = 0.42" / "p < 0.01"
- **Named entities:** researcher names, paper titles, venues, package/command names
...
- **Definitions Claude introduces itself** ("let tau denote the treatment effect")
...
Spawn `claim-verifier` via `Task` with `subagent_type=claim-verifier` and `context=fork`. Hand it: claims, verification questions, source material pointers. **Do not include the draft** -- forking removes the draft from the verifier's context automatically, but do not explicitly pass it either.
...
- This mirrors Pre-Flight's fail-closed contract. Hallucination discipline is most valuable precisely when things are going sideways -- that is when silent failures are most expensive.
...
- [`.claude/skills/verify-claims/SKILL.md`](../skills/verify-claims/SKILL.md) -- user-facing wrapper for ad-hoc verification of any text.
- [`content-invariants.md`](content-invariants.md) -- Pre-Flight (input side).
- [`cross-artifact-review.md`](cross-artifact-review.md) -- pattern-based; Post-Flight is draft-based.
- [`summary-parity.md`](summary-parity.md) -- rule against enumerative summaries drifting from their bodies; Post-Flight is the factual equivalent for draft content.
