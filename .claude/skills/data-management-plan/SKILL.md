...
Produce a Data Management Plan ready to paste into a funder portal. This skill writes the prose and structure; it does **not** submit anywhere. It is a **composition** skill — it folds the disclosure-avoidance / IRB rules from [`.claude/rules/confidential-data.md`](../../rules/confidential-data.md) and the environment + replication-package plan from [`/capture-environment`](../capture-environment/SKILL.md) and [`/replication-package`](../replication-package/SKILL.md) into a single funder-shaped document.
...
- **When restricted, licensed, or human-subjects data is involved.** The access/sharing and preservation sections change materially — see Phase 2. (Empirical-finance projects almost always touch licensed WRDS feeds; treat them like restricted data for the sharing decision.)
...
2. Classify the data on three axes (drives Phases 2–3):
   ...
   - **Restricted / licensed** (admin/tax/Census, proprietary panels, **WRDS vendor feeds: CRSP, Compustat, IBES, TAQ** under redistribution terms) — access procedures dominate.
   - **Human-subjects** (PII, biospecimen-linked, survey with identifiers) — IRB + disclosure avoidance dominate.

   If the data is restricted/licensed *or* human-subjects, set `sensitive = true` and run Phase 2. If it is purely public, Phase 2 is a short paragraph.
...
1. **Data description & types** — what data, source, volume, formats produced. Be specific: panel/admin microdata, RCT outcomes, event-study event files, replication intermediates (`.dta` / `.parquet` / `.sas7bdat`).
2. **Formats & metadata standards** — open/non-proprietary formats where possible (`.csv`/`.parquet` over `.dta`/`.sas7bdat`; codebooks; DDI / Dublin Core / domain schema). Name the standard, don't say "appropriate metadata".
3. **Storage & backup** — during the project: encrypted institutional storage, 3-2-1 backup, version control for code (not raw restricted/licensed data in git — `.gitignore` covers `data/raw/` and `*.dta`/`*.sas7bdat` microdata, per [`confidential-data.md`](../../rules/confidential-data.md)).
4. **Access & sharing** — who can access, when, under what terms. For restricted/licensed data this is the **restricted-data access procedure** (see Phase 2).
...
- **Restricted / licensed data → describe the access path, not the data.** State the data provider, the DUA/restricted-use agreement (or the WRDS subscription), and how a replicator obtains access (e.g., FSRDC application, openICPSR restricted-access tier, a WRDS account + the named library and date range to re-pull). The data itself is *not* deposited; the *path to it* is.
...
- **Environment capture.** State that the computational environment will be captured (Stata version + `.do` ado dependencies, Python `requirements.txt` / `uv.lock`, SAS version + WRDS macros, container for the Python layer). Point to [`/capture-environment`](../capture-environment/SKILL.md) as the mechanism. AEA Data Editor / DCAS standards expect this.
- **Replication package.** Commit to depositing a replication package (code + non-restricted data + a master run script + README) in a trusted repository. Point to [`/replication-package`](../replication-package/SKILL.md) as the builder.
- **Repository choice** — match the data class:
  - Economics / finance / accounting → **openICPSR** (AEA's home; DCAS-compliant) or **Harvard Dataverse**.
  - Restricted / licensed data → openICPSR *restricted-access* tier or the provider's enclave (FSRDC); deposit code + metadata + the re-pull recipe, not the microdata/licensed rows.
  - Domain repos → field-specific (e.g., ICPSR proper, Zenodo for code) where the funder or community expects them.
...
If the draft cites a funder policy or standard by name/number (e.g., "per NIH NOT-OD-21-013", "DCAS v1"), invoke [`/verify-claims`](../verify-claims/SKILL.md) via `Task` to confirm the policy citation resolves. The forked claim-verifier never sees the draft. Surface any FAIL/PARTIAL.
...
✓ DMP draft saved: quality_reports/dmp/<file>.md
  Funder: <nsf|nih|erc|horizon>   Data class: <public|restricted/licensed|human-subjects>
...
- [`.claude/rules/confidential-data.md`](../../rules/confidential-data.md) — restricted/licensed-data / IRB / disclosure-avoidance rules folded in at Phase 2.
- [`.claude/skills/disclosure-check/SKILL.md`](../disclosure-check/SKILL.md) — pre-release disclosure scan the plan commits released outputs to.
- [`.claude/skills/capture-environment/SKILL.md`](../capture-environment/SKILL.md) — the environment-capture mechanism Phase 3 references.
- [`.claude/skills/replication-package/SKILL.md`](../replication-package/SKILL.md) — the replication-package builder Phase 3 commits to.
