...
| **`nih`** | Specific Aims (1 p.) - Research Strategy (Significance/Innovation/Approach) - Human Subjects (if any) - Data Management & Sharing - Facilities & Other Resources - Budget Justification | Aims page is load-bearing |
...
Empirical-finance/economics framing is the primary lens (DiD/event-study, IV, panel asset-pricing tests; AEA Data Editor / openICPSR / DCAS data-sharing expectations), but the section scaffold is field-agnostic -- a biology or CS forker fills the same slots.
...
2. Locate the research spec: `--input <path>`, else the most recent `quality_reports/research_spec_*.md` from `/interview-me`. If none exists, **stop and recommend `/interview-me`** -- do not invent the science.
3. From the spec, extract: research question, hypotheses (directional), identification strategy (DiD / event study / IV / RDD / structural / asset-pricing test), data sources, sample, expected results, contribution. Record any `paper_type:` field.
...
- **Research Design & Methods** -- lift the identification strategy verbatim from the spec (estimand, treatment/control, identifying assumption, robustness: pre-trends, placebo, clustering). Name the estimator concretely (e.g. Stata `reghdfe` with firm + period FE, `csdid` / `did_multiplegt` for staggered DiD, `ivreghdfe` for IV; Python `linearmodels.PanelOLS`; SAS for the WRDS extract). See [`did-event-study`](../did-event-study/SKILL.md) for the design conventions.
...
- **Budget Justification skeleton** -- personnel / data acquisition (e.g. a licensed WRDS or vendor extract) / compute (HPC allocation) / travel / dissemination line-item stubs, each tied to an aim.
...
1. **Data Management (& Sharing) Plan** -- invoke [`/data-management-plan`](../data-management-plan/SKILL.md) via `Task` with the funder + data sources from the spec. It returns the DMP section (repository choice -- openICPSR / Dataverse / Zenodo, access/retention, FAIR/DCAS alignment). If any data source is sensitive (licensed WRDS extracts, restricted-use admin data, PII, IRB-restricted), have it honor [`.claude/rules/confidential-data.md`](../../rules/confidential-data.md) and describe access via a secure enclave / FSRDC / the vendor's license rather than open release. **Do not draft a sharing plan that promises to release confidential or licensed data.**
2. **Facilities / Computational-Environment statement** -- invoke [`/capture-environment`](../capture-environment/SKILL.md) via `Task` to produce the compute/software/dependency statement (the HPC cluster + SLURM allocation, the Stata / Python / SAS toolchain, the `requirements.txt` / `environment.yml` / ado-package-list / SAS-version provenance) for the Facilities section.
...
- **Methods <-> Budget** -- each cost line traces to an aim (a licensed-data aim implies a WRDS/vendor acquisition or enclave line; a large simulation or TAQ build implies an HPC compute line; an RA-intensive hand-collection implies a personnel line).
- **Aims <-> Timeline** -- every aim has at least one milestone; no milestone is unattributed.
- **DMP <-> Methods** -- the data named in Methods matches the data described in the DMP; confidential/licensed sources are not promised as open.
...
- **Post-flight (CoVe):** if Background/Significance cites prior literature, run the Post-Flight protocol from [`.claude/rules/post-flight-verification.md`](../../rules/post-flight-verification.md) -- spawn `claim-verifier` via `Task` (forked context) on the citations. Surface PASS / PARTIAL / FAIL. Skip on `--no-verify` or zero citations.
...
**Source spec:** quality_reports/research_spec_<slug>.md
...
- `--out <dir>` -- Output directory for the drafted sections (default `quality_reports/grants/YYYY-MM-DD_<slug>/`).
- `--no-verify` -- Skip the Phase 4 post-flight citation check (use when Background has no citations yet, or for a fast structural draft).
...
- [`.claude/skills/did-event-study/SKILL.md`](../did-event-study/SKILL.md) -- the DiD/event-study design conventions the Methods section names.
- [`.claude/rules/confidential-data.md`](../../rules/confidential-data.md) -- governs how sensitive/licensed data sources appear in the DMP and budget.
