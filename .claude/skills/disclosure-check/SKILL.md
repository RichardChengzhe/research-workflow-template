...
- `$0` — outputs directory to scan. Defaults to `output/` (this template's `output/tables/`, `output/figures/`, `output/logs/`). Recognised siblings: any export-staging directory (e.g., a `to_review/` folder the analyst stages for the RDC).
...
Glob the outputs dir for `.tex`, `.csv`, `.txt`, `.log`, `.smcl`, `.lst`, `.out`, `.md` tables and figure-data files. (Note: Stata logs are `.smcl`/`.log`; **SAS prints numeric output to `.lst`** — scan it, not just the `.log`.) For each:

- **Cell counts** — parse table cells / frequency columns; flag any count `0 < n < threshold` that is not already suppressed. `esttab`/`tabulate` count rows and `proc freq` outputs are the usual sources.
...
- **PII leakage** — regex for names, SSNs (`\d{3}-\d{2}-\d{4}`), exact dates of birth, addresses, exact lat/long or fine geocodes, record IDs (`gvkey`, `permno`, `cik`, internal `id`) that survived into a released output.
...
When two findings interact (a suppressed cell + a recoverable margin), report them **together** — the gate cares about the joint disclosure risk, not each cell in isolation. Be domain-aware: DiD / event-study cell counts per (cohort × period), IV first-stage subsamples, RCT arm × stratum balance tables, and panel firm-counts per industry × year are the usual offenders in finance/accounting panels.
...
- **Aggregate** — collapse thin categories, coarsen geography/industry, widen bins until every cell clears the threshold.
...
- [`.claude/skills/replication-package/SKILL.md`](../replication-package/SKILL.md) — the restricted-data deposit path this screen gates.
- [`.claude/skills/data-analysis/SKILL.md`](../data-analysis/SKILL.md) — produces the Stata / Python / SAS outputs this skill screens.
