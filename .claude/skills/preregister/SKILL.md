...
- **Psychology**, **political science**, broad social science → **OSF** (osf.io/registries) is the default. AsPredicted (aspredicted.org) is a popular short form for experiments.
- **Economics / finance field experiments** → **AEA RCT Registry** (socialscienceregistry.org) is mandatory for AEA journal submission since 2018.
...
1. **`--input <path>`** — a research spec produced by [`/interview-me`](../interview-me/SKILL.md) (saved under `quality_reports/specs/`) or any structured Markdown file. Read the spec and extract: research question, hypotheses (directional!), data source, design, sample, analysis approach. If the spec already has a `paper_type:` field (e.g., `survey-experiment`), use it to bias the style choice.
...
| Field experiment in econ/finance / labelled "RCT" / IRB-approved randomised intervention | `aea-rct` |
| 9-question quick-form ask, lab experiment, time-pressure | `aspredicted` |
| Anything else | `osf` |
...
- **Estimator named.** Analysis plan names a specific estimator (OLS / `reg`, `reghdfe` two-way FE, `xtreg, fe`, logit, `csdid` ATT, IV / `ivreg2`, difference-in-means …) and a primary outcome variable. "Regression" alone is insufficient. Name the SE/clustering too — cluster level is a researcher degree of freedom (see [`inference-robustness.md`](../../rules/inference-robustness.md)).
...
- **Exclusions ex ante.** Outlier and exclusion rules are stated *before* the data is seen ("we will winsorize continuous variables at 1/99"; "we will exclude observations with completion time < 1 minute"). Vague "we'll deal with outliers" fails.
- **Internal consistency.** If the design is randomised, the unit of randomisation matches the unit of analysis OR the analysis plan addresses clustering. If observational, the identification strategy is stated (e.g. staggered DiD with not-yet-treated controls — then cross-ref [`did-conventions.md`](../../rules/did-conventions.md)).
...
If the document cites prior literature in the rationale section (e.g., "Building on Smith et al. 2014, we expect …"), invoke [`/verify-claims`](../verify-claims/SKILL.md) via `Task` to fact-check those citations. Pass the draft path and a list of explicit citations. The forked claim-verifier (never sees the draft) returns PASS / PARTIAL / FAIL per citation. Surface any FAIL/PARTIAL in the output summary.

Skip post-flight if:

- No prior-literature citations in the document.
- User passes `--no-verify`.
...
- [`.claude/skills/interview-me/SKILL.md`](../interview-me/SKILL.md) — produces the spec this skill consumes via `--input`.
- [`.claude/skills/power-analysis/SKILL.md`](../power-analysis/SKILL.md) — supplies the MDE / required-N + power paragraph for the sample-plan field (RCTs).
- [`.claude/skills/verify-claims/SKILL.md`](../verify-claims/SKILL.md) — Phase 5 invokes this for citation post-flight.
- [`.claude/rules/inference-robustness.md`](../../rules/inference-robustness.md) — commit the multiple-testing family + correction in advance (the unit of correction is itself a researcher degree of freedom).
- [`.claude/rules/replication-protocol.md`](../../rules/replication-protocol.md) — preregistration is the *forward* commitment; replication-protocol is the *backward* contract.

## Examples

### Example 1 — Survey experiment from a spec
...
2. Extract 2 directional hypotheses, online panel N=1,200, OLS with treatment dummies.
...
3. Cross-check: estimator (cluster-robust OLS, `reghdfe ... vce(cluster village)`), primary outcome stated, exclusion rule (intent-to-treat) stated. Pass.
4. Run `/power-analysis --design cluster` for the MDE paragraph.
...
**User says:** "Preregister my next experiment, AsPredicted style" (no `--input`)
**Actions:**
