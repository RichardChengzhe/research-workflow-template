...
argument-hint: "[manuscript path] [journal short-name, e.g. JF] [--no-ai | --statements-only]"
...
1. If a journal short-name is given, read its profile in [`journal-profiles.md`](../../references/journal-profiles.md) (top econ + AEA-imprint policy notes + finance top-3: JF, RFS, JFE).
2. **Verify the current policy on the journal's own site** (`WebSearch`/`WebFetch`: "<journal> artificial intelligence policy authors", the journal's submission-guidelines page). Policies moved fast in 2025–2026; a cached or remembered policy is not good enough for a submission. **Fetch at draft time** and record the URL and retrieval date in the output.
...
- **What stayed human**: research design, identification choices, interpretation, final verification of every number and citation (tie to the repo's own verification story — [`/audit-reproducibility`](../audit-reproducibility/SKILL.md), [`/verify-claims`](../verify-claims/SKILL.md) — when true, *say so*: "all AI-assisted numbers were independently verified against code" is a strength, not a confession).
...
4. **Data availability** — aligned with the replication deposit: openICPSR/DCAS language when the target is an AEA-imprint journal (delegate the deposit itself to [`/replication-package`](../replication-package/SKILL.md); restricted/licensed-data access language per [`confidential-data.md`](../../rules/confidential-data.md) — e.g. WRDS feeds are "available to subscribers; not redistributed").
...
- [`.claude/rules/confidential-data.md`](../../rules/confidential-data.md) — restricted/licensed-data constraints on what the statements can say.
