---
name: humanize
description: Read-only audit of `.tex` or `.md` text for AI-voice tells — boilerplate transitions ("Moreover", "Furthermore", "It is important to note that"), AI-cliche lexicon ("delve", "navigate the complexities", "tapestry", "robust framework"), em-dash overuse, symmetric paragraph shapes, tricolon abuse, hedging stacking, "not only X but also Y" frames, and formulaic openers. Produces a report; does NOT rewrite. Use when user says "humanize", "does this sound like AI?", "check for AI tells", "de-AI this draft", "remove AI voice", "audit my prose for sycophancy", or before journal submission / posting a working paper.
...
Finance and accounting referees and editors increasingly recognise AI-generated prose. The tells are not stylistic preferences — they're statistically conspicuous patterns the LLM training distribution produces at higher rates than human academic writers. Five reasons to audit before submission:

1. **Reviewer suspicion is a tax.** Even good substance pays a credibility tax if the prose reads as AI-drafted.
2. **Journal policy is tightening.** A growing number of venues require disclosure of AI assistance or prohibit AI-drafted text.
3. **AI tells signal weak content.** Boilerplate transitions ("Moreover", "It is important to note") almost always cover up logical gaps the author didn't think through — exactly what a sharp referee probes.
...
- On `.bib`, `.do`, `.py`, `.sas`, or other non-prose files — the detectors are tuned for academic prose.
- On code comments — the tells are different.
- On a results table's `\textit{Notes:}` block (terse, conventional) — voice norms diverge.

## Detection categories

The `humanize-auditor` agent checks these category groups. The agent is calibrated to finance/accounting: it does **not** flag legitimate technical terms in this field (`leverage` the noun, `robust standard errors`, `significant` meaning statistically significant).
...
- "leverage" **only** as a management-speak verb ("leverage synergies") — NOT the finance noun (financial leverage, leverage ratio), which is legitimate here
...
**Severity:** HIGH — these are almost never authorial choices; they're LLM uncertainty-management. (Note: a *single* calibrated hedge is correct scientific writing — "the estimate is consistent with"; flag the *stack*, not honest uncertainty.)
...
- Section openers of the form "This [paper / section / analysis] [does X]."
- Paragraph openers that re-state the section title.
- Abstract opening with "In this paper, we..." — legitimate in some sub-fields; flag for review where it's atypical (top finance/econ abstracts rarely open this way).
...
1. **Identify files to audit:**
   ...
   - If `$ARGUMENTS` is `all`: audit prose files — `.tex` under `manuscript/` (resolve `\input{}`/`\include{}`), and `.md` in the repo root, `docs/`, and `master_supporting_docs/`.
   - Skip `.bib`, `.do`, `.py`, `.sas`, code files, and anything under `scripts/`, `code/`, `output/`.
...
| Situation | Run |
| --- | --- |
| Drafted prose with AI assistance | `/humanize` before submission, paired with `/proofread` (grammar) and `/verify-claims` (citations). |
| Wrote in your own voice | `/humanize` anyway — your own prose drifts toward LLM patterns after long sessions of AI-assisted work. |
...
We deliberately do not ship `/humanize --rewrite`. Cross-vendor research finds that auto-rewriting prose to strip AI tells degrades quality more often than it improves it — the rewriter introduces its *own* AI tells. The detect-and-flag pattern preserves authorial voice; the cost is your editing time, which is exactly the cost we want to pay.
...
- Report at `quality_reports/humanize_<filename>_report.md`.
...
- `.claude/agents/humanize-auditor.md` — the read-only auditor this skill drives (finance/accounting-calibrated).
- `.claude/skills/proofread/SKILL.md` — grammar/typo/overflow lens (different from voice).
- `.claude/skills/verify-claims/SKILL.md` — citation + numeric fact-checking lens.
- `.claude/skills/review-paper/SKILL.md` — substance/identification/contribution lens.
